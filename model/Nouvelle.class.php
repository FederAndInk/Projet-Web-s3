...
class Nouvelle
{
 ...
 // Contenu de la nouvelle
  private $url;
 // Le lien vers la ressource associée à la nouvelle
  private $urlImage;
 // URL vers l'image
  ...
  function downloadImage(DOMElement $item, $imageId)
  {
    ...
    // L'attribut url a été trouvé : on récupère sa valeur, c'est l'URL de
    // l'image
    $url = $item->getElementsByTagName('enclosure')
      ->item(0)
      ->getAttribute('url');
    ...
    if ($url != NULL && $bool) {
      // On construit un nom local pour cette image : on suppose que
      // $nomLocalImage contient un identifiant unique
