...
class DAO {
  ...
  */
  ...
  function deleteRSSUtilisateur(string $user,int $RSS_id){
    $test = null;
    $rqt = "DELETE FROM abonnement WHERE RSS_id = '$RSS_id' and utilisateur_login='$user'"; // on supprime l'abonnement de l'utilisateur
    $result = $this->db->exec($rqt);

    $rqt = "SELECT * FROM abonnement WHERE RSS_id = '$RSS_id'";
    $result = $this->db->query ( $rqt )->fetchAll ( PDO::FETCH_ASSOC );
    if(empty($result)){ // Si il n'y a personne d'abonné au flux on le suprimme de la base de données
      $test = 'ok';
