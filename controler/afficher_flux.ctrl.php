  ...
  if (isset($_GET['new_flux'])) {
    ...
    if (filter_var($new_flux, FILTER_VALIDATE_URL)) { // On vérifie que c'est
      ...
      if ($rss1 == NULL) {
        ...
        $dao->addFluxUtilisateur($rss1->id(), $user); // on ajoute le flux aux
                                                     // abonnements de
                                                     // l'utilisateur
      } else {
        $dao->addFluxUtilisateur($rss1->id(), $user); // on ajoute le flux aux
      ...
      $message_erreur_addRSS = "Le flux n'existe pas"; // message d'erreur
                                                       // envoyé à la view si ce
                                                       // n'est pas un lien
    }
  }
  ...
  if (isset($_GET['mot_clef'])) {
    ...
    header("Location: afficher_nouvelles.ctrl.php?mot_clef=$motClef"); // Si
                                                                       // aucun
                                                                       // mot
                                                                       // clé ne
                                                                       // correspond
  ...
  if (isset($_GET['supr_Id'])) {
    // on supprime le RSS de l'abonnement de l'utilisateur, et on le supprimme
    // de la base s'y perssone n'y est abonné
